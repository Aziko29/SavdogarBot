...
- Waiting note: `INQUIRY_WAIT_MIN` (default 7, range 0-240, `0` = off): how long a customer's question may stay unaccepted before he is told the admins are busy (or, at night, that the answer comes in the morning); see section 4.
...
- **Saved details:** after a confirmed order the bot remembers the customer's phone and delivery address (never shown to admins beyond the order itself). On his next order, right after the quantity, he is offered "Ha, shular" / "O'zgartirish" and can skip the phone and address steps. A "pick it up myself" order does not overwrite a saved address. `/malumotlarim` shows what is stored and deletes it on request.
- **Command menu and help:** the bot publishes the Telegram "/" menu (`bot_commands.py`, at every start). Customers see **/buyurtmalarim**, **/malumotlarim**, **/cancel** and **/yordam**; each admin gets his own menu (**/admin**, **/cancel**, **/yordam**) through a per-chat scope, which is set at startup, when the head admin adds him, and on his `/start` (a chat that has never pressed Start cannot get a menu yet), and taken away when he is removed. `/yordam` (or `/help`) explains how to order and lists the commands (admins get a short admin version); it works in any state, also in the middle of the order form, and never cancels anything. The plain `/start` greeting points to the menu and `/yordam`. A menu that Telegram refuses to store is only logged, never fatal.
...
- **Customer notes:** when an admin accepts a chat with the **Qabul qilish** button, the customer gets "Admin savolingizni ko'rdi, tez orada javob yozadi" (if he has not written yet: "Admin suhbatga qo'shildi. Savolingizni yozing."); accepting by replying sends nothing extra, the reply itself is the answer. If nobody accepts within `INQUIRY_WAIT_MIN` minutes of his first message, he gets one note: "Hozir adminlar band..." by day, or "Hozir tungi vaqt. Savolingizga ertalab javob beramiz." inside the night window from `/admin`. Sent once per opened inquiry, never to an already accepted chat; the check runs every minute (`inquiry_wait.py`).
...
Existing databases are upgraded automatically at startup (schema v6 adds the new order columns, v7 the inquiry tables, v8-v9 the inquiry claim columns and the card/transcript tables, v10 the saved customer details, v11 the customer waiting note flag); old orders show quantity 1 and no phone/address.
